...
app.use(session({ secret: SESSION_SECRET, resave: false, saveUninitialized: false, cookie: { httpOnly: true, sameSite: 'lax', maxAge: 1000 * 60 * 60 * 24 * 30 } }));
...
function encodeNonAscii(html=''){ return String(html); }
...
async function requireUser(req,res,next){ let u = req.session.userId ? await store.getUserById(req.session.userId) : null; if(!u && req.cookies && req.cookies.auth_token){ const tok=readAuthToken(req.cookies.auth_token); if(tok){ u=await store.getUserById(tok.id); if(u){ req.session.userId=u.id; req.session.userEmail=u.email; } } } if(u){ req.user=u; return next(); } res.redirect(303, '/admin/login'); }
function slugify(s){ return String(s||'exam').trim().toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9\-\u0600-\u06FF]/g,'').slice(0,60) || nanoid(6); }
function normalizeEmail(email){ return String(email || '').trim().toLowerCase(); }
function makeAuthToken(user){ const payload = `${user.id}|${user.email}`; const sig = crypto.createHmac('sha256', SESSION_SECRET).update(payload).digest('hex'); return Buffer.from(`${payload}|${sig}`).toString('base64url'); }
function readAuthToken(token){ try{ const raw = Buffer.from(String(token||''), 'base64url').toString('utf8'); const parts = raw.split('|'); if(parts.length !== 3) return null; const [id,email,sig]=parts; const payload=`${id}|${email}`; const good=crypto.createHmac('sha256', SESSION_SECRET).update(payload).digest('hex'); if(!crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(good))) return null; return {id,email}; }catch(e){ return null; } }
function setAuthCookie(res,user){ res.cookie('auth_token', makeAuthToken(user), { httpOnly:true, sameSite:'lax', secure:true, maxAge:1000*60*60*24*30 }); }
function clearAuthCookie(res){ res.clearCookie('auth_token'); }
...
app.post('/admin/account',requireUser,asyncRoute(async(req,res)=>{ const email=normalizeEmail(req.body.email); const other=await store.getUserByEmail(email); if(other&&other.id!==req.user.id) return res.send(layout('خطأ',adminNav(req)+`<div class="error">الإيميل مستخدم في حساب آخر.</div>`)); await store.updateUserEmail(req.user.id,email); req.session.userEmail=email; setAuthCookie(res,{id:req.user.id,email}); res.redirect(303, '/admin/account'); }));
