...
function publicBaseUrl(req){ const env=process.env.BASE_URL; if(env && !String(env).includes('localhost')) return String(env).replace(/\/$/,''); const proto=String(req.headers['x-forwarded-proto'] || req.protocol || 'https').split(',')[0]; return `${proto}://${req.headers.host}`; }
