...
async function requireUser(req,res,next){ const u = req.session.userId ? await store.getUserById(req.session.userId) : null; if(u){ req.user=u; return next(); } res.redirect('/admin/login'); }
...
app.post('/admin/account',requireUser,asyncRoute(async(req,res)=>{ const email=normalizeEmail(req.body.email); const other=await store.getUserByEmail(email); if(other&&other.id!==req.user.id) return res.send(layout('خطأ',adminNav(req)+`<div class="error">الإيميل مستخدم في حساب آخر.</div>`)); await store.updateUserEmail(req.user.id,email); req.session.userEmail=email; res.redirect('/admin/account'); }));
...
app.post('/admin/exams/:id/questions',requireUser,asyncRoute(async(req,res)=>{ const exam=await getOwnedExam(req,res); if(!exam)return; const options=[req.body.opt0,req.body.opt1,req.body.opt2,req.body.opt3].filter(x=>String(x||'').trim()); const correctIndex=Number(req.body.correctIndex); if(options.length<2||correctIndex>=options.length) return res.send(layout('خطأ',adminNav(req)+`<div class="error">لازم على الأقل اختيارين، والإجابة الصحيحة تكون ضمن الاختيارات المكتوبة.</div>`)); exam.questions.push({id:nanoid(8),text:req.body.text,options,correctIndex,points:Number(req.body.points||1)}); await store.updateExam(exam); res.redirect(`/admin/exams/${exam.id}`); }));
app.post('/admin/exams/:id/questions/:qid/delete',requireUser,asyncRoute(async(req,res)=>{ const exam=await getOwnedExam(req,res); if(!exam)return; exam.questions=exam.questions.filter(q=>q.id!==req.params.qid); await store.updateExam(exam); res.redirect(`/admin/exams/${exam.id}`); }));
app.post('/admin/exams/:id/toggle',requireUser,asyncRoute(async(req,res)=>{ const exam=await getOwnedExam(req,res); if(!exam)return; exam.published=!exam.published; await store.updateExam(exam); res.redirect(req.get('referer')||'/admin'); }));
app.post('/admin/exams/:id/delete',requireUser,asyncRoute(async(req,res)=>{ await store.deleteExam(req.params.id,req.user.id); res.redirect('/admin'); }));
