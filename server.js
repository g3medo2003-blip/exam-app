...
`;
  ...
  const bottom = `<nav class="bottom-nav"><a href="/admin">${icon('home')}<span>الرئيسية</span></a><a href="/admin/exams/new">${icon('plus')}<span>امتحان</span></a><a href="/admin/results">${icon('chart')}<span>النتائج</span></a><a href="/admin/account">${icon('user')}<span>حسابي</span></a></nav>`;
  const html = `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>${esc(title)}</title><style>${css}</style></head><body><div class="ai-bg"><div class="orb a"></div><div class="orb b"></div><div class="orb c"></div></div><div class="grid-bg"></div><main class="container">${body}<footer class="footer">تم التطوير بواسطة ahmed</footer></main>${bottom}<div id="toast" class="toast"></div>${js}</body></html>`;
  return encodeNonAscii(html);
}
function adminNav(req){ return `<div class="nav"><a href="/admin">${icon('home')} الرئيسية</a><a href="/admin/exams/new">${icon('plus')} امتحان جديد</a><a href="/admin/results">${icon('chart')} النتائج</a><a href="/admin/account">${icon('user')} حسابي</a><a class="btn secondary" href="/admin/logout">${icon('logout')} خروج</a></div><p class="muted small">مسجل دخول: ${esc(req.session.userEmail || '')}</p>`; }
