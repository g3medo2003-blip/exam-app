...
function layout(title, body){
  ...
  const js = `<script>function copyLink(t){navigator.clipboard&&navigator.clipboard.writeText(t).then(function(){toast('تم نسخ الرابط')}).catch(function(){prompt('انسخ الرابط',t)})}function toast(t){var e=document.getElementById('toast');if(!e)return;e.textContent=t;e.style.display='block';setTimeout(function(){e.style.display='none'},1800)}</script>`;
...
app.get('/admin',requireUser,asyncRoute(async(req,res)=>{ const exams=await store.listExams(req.user.id); const cards=exams.map(e=>{ const link=`${BASE_URL}/exam/${e.slug}`; return `<div class="card"><h2>📝 ${esc(e.title)}</h2><p><span class="badge">${e.questions.length} سؤال</span> <span class="badge">${e.published?'منشور':'غير منشور'}</span></p><div class="linkbox ltr"><a href="/exam/${encodeURIComponent(e.slug)}" target="_blank">${esc(link)}</a></div><div class="row"><button type="button" class="btn ok" onclick="copyLink('${esc(link)}')">📋 نسخ رابط الامتحان</button><a class="btn" href="/admin/exams/${e.id}">⚙️ تعديل الأسئلة</a><form method="post" action="/admin/exams/${e.id}/toggle"><button class="btn secondary">${e.published?'⏸️ إيقاف النشر':'🚀 نشر'}</button></form><form method="post" action="/admin/exams/${e.id}/delete" onsubmit="return confirm('حذف الامتحان؟')"><button class="btn danger">🗑️ حذف</button></form></div></div>` }).join('')||'<div class="card"><p>لا يوجد امتحانات بعد.</p></div>'; res.send(layout('لوحة التحكم',adminNav(req)+`<h1>✨ لوحة التحكم</h1><div class="hero"><div class="notice">النتائج محفوظة حتى بعد تسجيل الخروج، ولن يتم حذفها إلا عندما تضغط حذف بنفسك.</div></div>${cards}`)); }));
